...
public class CredentialDAO {
    ...
    /**
     * Retrieves the primary role name for a given EmployeeID (UserID). This
     * method assumes Admin > Manager > Regular Employee hierarchy.
     *
     * @param employeeId The EmployeeID (UserID) to find the primary role for.
     * @return The primary RoleName (e.g., "Admin", "Manager", "Regular
     * Employee") or null if no role is found.
    ...
    /**
     * Inserts a new user into the 'user' table using a provided connection.
     *
     ...
     * @return The auto-generated UserID as a String, or null if insertion
     * fails.
     * @throws SQLException if a database access error occurs.
     */
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) { //
            ...
            if (rowsAffected > 0) { //
                try (ResultSet rs = pstmt.getGeneratedKeys()) { //
                    if (rs.next()) { //
                        generatedUserId = String.valueOf(rs.getInt(1)); // Get the auto-generated UserID
                    }
                }
            }
        } catch (SQLException e) { //
    ...
    /**
     * Assigns a role to a user in the 'userrole' table using a provided
     * connection.
     *
     * @param conn The database connection to use for the transaction.
     * @param userId The UserID (EmployeeID) to assign the role to.
     * @param roleName The name of the role (e.g., "Admin", "Manager", "Regular
     * Employee").
     * @return true if role assignment was successful, false otherwise.
     * @throws SQLException if a database access error occurs.
     */
    public boolean assignRoleToUser(Connection conn, String userId, String roleName) throws SQLException { //
        // First, get the RoleID from the role name (can use its own connection as it's read-only)
        String getRoleIdSql = "SELECT RoleID FROM role WHERE RoleName = ? AND IsDeleted = 0"; //
        int roleId = -1; //
        try (Connection localConn = DBConnection.getConnection(); // Gets its own connection for lookup
        ...
        // Now insert into userrole table - USE THE PROVIDED CONNECTION
